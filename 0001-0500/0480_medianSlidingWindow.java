...
public class RedBlackTree<T extends Comparable<T>> {
    ...
    public double getMedian() {
        ...
        if (size % 2 == 1) {
            T middleValue = getNthNodeInOrder(root, size / 2 + 1).key;
            return toDouble(middleValue);
        } else {
            T firstMiddle = getNthNodeInOrder(root, size / 2).key;
            T secondMiddle = getNthNodeInOrder(root, size / 2 + 1).key;
            return (toDouble(firstMiddle) + toDouble(secondMiddle)) / 2.0;
        }
    }

    private RedBlackNode<T> getNthNodeInOrder(RedBlackNode<T> node, int n) {
        if (isNil(node)) {
            return null;
        }

        int leftSubtreeSize = node.numLeft + 1;

        if (n == leftSubtreeSize)
            return node;
        else if (n < leftSubtreeSize)
            return getNthNodeInOrder(node.left, n);
        else
            return getNthNodeInOrder(node.right, n - leftSubtreeSize);
    }
...
class Solution {
    public double[] medianSlidingWindow(int[] nums, int k) {
        ...
        for (int i = 0; i < nums.length; i++) {
            ...
            medians[idx] = tree.getMedian();
            idx++;
            RedBlackNode<Integer> nodeToRemove = tree.search(nums[i - k + 1]);
